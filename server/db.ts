...
function formatDoc(doc: any) {
  ...
  if (typeof formatted.views !== "number") {
    formatted.views = 0;
  }
  return formatted;
}
...
// -------------------------------------------------------------
// ANALYTICS & READERSHIP TRACKING (Daily Platform & Section Metrics)
// -------------------------------------------------------------
...
// Track whether we've purged old synthetic records on startup
let hasPurgedOldSeed = false;
async function purgeOldSyntheticAnalytics(database: Db) {
  if (hasPurgedOldSeed) return;
  try {
    // Delete documents generated by old synthetic seeds (which had totalViews > 1000 or the deprecated seed fields)
    const res = await database.collection("daily_analytics").deleteMany({
      $or: [
        { totalViews: { $gt: 1000 } },
        { views: { $exists: true } },
      ],
    ...
    if (res.deletedCount > 0) {
      console.log(`[Analytics] Purged ${res.deletedCount} old synthetic seed records.`);
    }
    hasPurgedOldSeed = true;
  } catch (e) {
    console.error("[Analytics] Error purging old synthetic analytics:", e);
  }
}

export async function recordDailyPlatformView(data: {
  type?: string; // 'page' | 'article' | 'spotlight'
  id?: string;
  category?: string;
  path?: string;
  visitorId?: string;
  ...
  const database = await getDb();
  const vId = (data.visitorId || "guest").slice(0, 64);

  if (database) {
    try {
      await purgeOldSyntheticAnalytics(database);
      const col = database.collection("daily_analytics");

      const incFields: any = { totalViews: 1 };
      if (data.type === "article") {
        incFields.articleViews = 1;
      } else if (data.type === "spotlight") {
        incFields.spotlightViews = 1;
      } else {
        incFields.otherViews = 1;
      }

      const updateOp: any = {
        $inc: incFields,
        $set: { updatedAt: new Date().toISOString() },
        $addToSet: { visitorIds: vId },
      };

      if (data.category) {
        updateOp.$set.topCategory = data.category;
      }

      await col.updateOne({ date: todayStr }, updateOp, { upsert: true });

      // Keep uniqueVisitors count strictly synchronized with distinct visitor IDs
      const updatedDoc = await col.findOne({ date: todayStr });
      if (updatedDoc && Array.isArray(updatedDoc.visitorIds)) {
        await col.updateOne(
          { date: todayStr },
          { $set: { uniqueVisitors: updatedDoc.visitorIds.length } }
        );
      }
      return;
    } catch (e) {
      console.error("[MongoDB] Error recording real daily view:", e);
    }
  }

  // Fallback in-memory DB
  let fallbackToday = fallbackDb.dailyAnalytics.find((d: any) => d.date === todayStr);
  if (!fallbackToday) {
    fallbackToday = {
      date: todayStr,
      totalViews: 1,
      uniqueVisitors: 1,
      visitorIds: [vId],
      articleViews: data.type === "article" ? 1 : 0,
      spotlightViews: data.type === "spotlight" ? 1 : 0,
      otherViews: data.type !== "article" && data.type !== "spotlight" ? 1 : 0,
      topCategory: data.category || "General",
      updatedAt: new Date().toISOString(),
    };
    fallbackDb.dailyAnalytics.push(fallbackToday);
  } else {
    fallbackToday.totalViews = (fallbackToday.totalViews || 0) + 1;
    if (data.type === "article") {
      fallbackToday.articleViews = (fallbackToday.articleViews || 0) + 1;
    } else if (data.type === "spotlight") {
      fallbackToday.spotlightViews = (fallbackToday.spotlightViews || 0) + 1;
    } else {
      fallbackToday.otherViews = (fallbackToday.otherViews || 0) + 1;
    }
    if (!fallbackToday.visitorIds) fallbackToday.visitorIds = [];
    if (!fallbackToday.visitorIds.includes(vId)) {
      fallbackToday.visitorIds.push(vId);
      fallbackToday.uniqueVisitors = fallbackToday.visitorIds.length;
    }
    if (data.category) {
      fallbackToday.topCategory = data.category;
    }
    fallbackToday.updatedAt = new Date().toISOString();
  }
}

export async function incrementArticleViews(idOrSlug: string, count: number = 1, visitorId?: string): Promise<number> {
  ...
  if (database) {
    try {
      ...
      if (objId) query.$or.push({ _id: objId });

      const doc = await database.collection("articles").findOne(query);
      if (doc) {
        const currentViews = typeof doc.views === "number" ? doc.views : 0;
        updatedViews = currentViews + count;
        await database.collection("articles").updateOne(
          { _id: doc._id },
          { $set: { views: updatedViews, lastReadAt: new Date().toISOString() } }
        );
        await recordDailyPlatformView({ 
          type: "article", 
          id: doc._id?.toString(), 
      ...
      console.error("[MongoDB] Error incrementing real article views:", e);
    }
  }

  // Fallback DB
  const fallback = fallbackDb.articles.find(
    (a) =>
      (a.slug && a.slug.toLowerCase() === cleanKey) ||
      a.id === idOrSlug ||
      a._id === idOrSlug ||
  ...
  if (fallback) {
    const currentViews = typeof fallback.views === "number" ? fallback.views : 0;
    fallback.views = currentViews + count;
    fallback.lastReadAt = new Date().toISOString();
    updatedViews = fallback.views;
    await recordDailyPlatformView({ 
      type: "article", 
      id: fallback.id, 
  ...
  await recordDailyPlatformView({ type: "article", id: idOrSlug, visitorId });
  return 1;
}

export async function incrementSpotlightViews(idOrSlug: string, count: number = 1, visitorId?: string): Promise<number> {
  if (!idOrSlug) return 0;
  const cleanKey = idOrSlug.trim().toLowerCase();
  const database = await getDb();
  let updatedViews = 0;
  ...
  if (database) {
    try {
      ...
      if (doc) {
        const currentViews = typeof doc.views === "number" ? doc.views : 0;
        updatedViews = currentViews + count;
        await database.collection("spotlights").updateOne(
          { _id: doc._id },
          { $set: { views: updatedViews, lastReadAt: new Date().toISOString() } }
        );
        await recordDailyPlatformView({ 
      ...
      console.error("[MongoDB] Error incrementing real spotlight views:", e);
    }
  }
  ...
  if (fallback) {
    ...
    fallback.lastReadAt = new Date().toISOString();
    updatedViews = fallback.views;
    await recordDailyPlatformView({ type: "spotlight", id: fallback.id, visitorId });
    return updatedViews;
  }

  await recordDailyPlatformView({ type: "spotlight", id: idOrSlug, visitorId });
  return 1;
}

export async function getDailyAnalyticsFromDb(daysCount: number = 30): Promise<any[]> {
  const database = await getDb();
  ...
  for (let i = daysCount - 1; i >= 0; i--) {
    ...
    dateList.push(getFormattedDate(d));
  }
  ...
  if (database) {
    try {
      await purgeOldSyntheticAnalytics(database);
      const col = database.collection("daily_analytics");
      dbRecords = await col.find({ date: { $in: dateList } }).toArray();
    } catch (e) {
      console.error("[MongoDB] Error getting real daily analytics:", e);
    }
  } else {
    dbRecords = (fallbackDb.dailyAnalytics || []).filter((d: any) => dateList.includes(d.date));
  }
  ...
  // Return strictly accurate calendar records for each date (showing 0 on inactive days)
  return dateList.map((dateStr) => {
    const existing = recordMap.get(dateStr);
    if (existing) {
      const totalViews = existing.totalViews || 0;
      const uniqueVisitors = typeof existing.uniqueVisitors === "number"
        ? existing.uniqueVisitors
        : (Array.isArray(existing.visitorIds) ? existing.visitorIds.length : (totalViews > 0 ? 1 : 0));

      return {
        date: dateStr,
        totalViews,
        uniqueVisitors,
        articleViews: existing.articleViews || 0,
        spotlightViews: existing.spotlightViews || 0,
        otherViews: existing.otherViews || 0,
        topCategory: existing.topCategory || "General",
...
export async function getPlatformAnalytics() {
  const [articles, spotlights, dailyHistory] = await Promise.all([
    getArticlesFromDb(),
    getSpotlightsFromDb(),
    getDailyAnalyticsFromDb(30),
  ]);

  const todayStr = getFormattedDate(new Date());
  const todayDoc = dailyHistory.find((d: any) => d.date === todayStr) || {
    totalViews: 0,
    uniqueVisitors: 0,
    articleViews: 0,
    spotlightViews: 0,
  ...
  const yesterdayDoc = dailyHistory.find((d: any) => d.date === yesterdayStr) || {
    totalViews: 0,
    uniqueVisitors: 0,
    articleViews: 0,
    spotlightViews: 0,
  ...
  const last7DaysViews = last7Days.reduce((acc: number, cur: any) => acc + (cur.totalViews || 0), 0);
  const prev7Days = dailyHistory.slice(-14, -7);
  const prev7DaysViews = prev7Days.reduce((acc: number, cur: any) => acc + (cur.totalViews || 0), 0);
  const growthRate = prev7DaysViews > 0 
    ? Math.round(((last7DaysViews - prev7DaysViews) / prev7DaysViews) * 100) 
    : (last7DaysViews > 0 ? 100 : 0);

  const last30DaysViews = dailyHistory.reduce((acc: number, cur: any) => acc + (cur.totalViews || 0), 0);
  const activeDays = dailyHistory.filter((d: any) => (d.totalViews || 0) > 0).length;
  const avgDailyViews = activeDays > 0 ? Math.round(last30DaysViews / activeDays) : 0;
  
  // Real platform totals: exact sums of actual recorded article views + spotlight views
  const totalAllArticleViews = articles.reduce((acc: number, a: any) => acc + (a.views || 0), 0);
  const totalAllSpotlightViews = spotlights.reduce((acc: number, s: any) => acc + (s.views || 0), 0);
  
  // Real total platform views = actual sum of all daily total views recorded across time (or sum of all real article + spotlight reads)
  const totalPlatformViews = Math.max(last30DaysViews, totalAllArticleViews + totalAllSpotlightViews);

  // Group articles by section/category
  const sectionMap = new Map<string, any[]>();
  for (const a of articles) {
    const cat = (a.category || "General").trim();
    if (!sectionMap.has(cat)) {
      sectionMap.set(cat, []);
    }
    sectionMap.get(cat)!.push(a);
  }

  const sectionsList: any[] = [];
  sectionMap.forEach((categoryArticles, catName) => {
    // Sort articles within category by views descending
    const sorted = [...categoryArticles].sort((a, b) => (b.views || 0) - (a.views || 0));
    const sectionViews = sorted.reduce((acc, a) => acc + (a.views || 0), 0);
    const avgViews = Math.round(sectionViews / Math.max(1, sorted.length));
    const shareOfTotal = totalAllArticleViews > 0 ? Number(((sectionViews / totalAllArticleViews) * 100).toFixed(1)) : 0;

    const enrichedArticles = sorted.map((art) => {
      const shareOfCategory = sectionViews > 0 ? Number((((art.views || 0) / sectionViews) * 100).toFixed(1)) : 0;
      return {
        id: art.id || art._id,
        title: art.title,
        slug: art.slug,
        ...
        author: art.author,
        authorDesignation: art.authorDesignation,
    ...
    sectionsList.push({
      ...
      sharePercentage: shareOfTotal,
      topArticle: enrichedArticles[0] || null,
      articles: enrichedArticles,
    });
  });

  // Sort sections by totalViews descending
  sectionsList.sort((a, b) => b.totalViews - a.totalViews);

  // Spotlights breakdown
  const sortedSpotlights = [...spotlights].sort((a, b) => (b.views || 0) - (a.views || 0));
  const enrichedSpotlights = sortedSpotlights.map((spot) => {
    const shareOfSpotlights = totalAllSpotlightViews > 0 
      ? Number((((spot.views || 0) / totalAllSpotlightViews) * 100).toFixed(1))
      : 0;
    return {
      id: spot.id || spot._id,
      founderName: spot.founderName,
      ...
      views: spot.views || 0,
      shareOfSpotlights,
      createdAt: spot.createdAt || spot.publishedAt,
    };
  });

  // Top overall articles
  ...
  // Chronological 30-day daily records for charts
  const chronologicalHistory = [...dailyHistory]
    ...
    .map((d: any) => {
      ...
      return {
        ...
        formattedDate: `${monthNames[dt.getMonth()]} ${dt.getDate()}`,
      };
    });
  ...
  return {
    platform: {
      ...
      todayViews: todayDoc.totalViews || 0,
      yesterdayViews: yesterdayDoc.totalViews || 0,
      last7DaysViews,
      last30DaysViews,
      avgDailyViews,
      todayUniqueVisitors: todayDoc.uniqueVisitors || 0,
      growthRate,
      totalArticleReads: totalAllArticleViews,
      totalSpotlightReads: totalAllSpotlightViews,
    },
    dailyHistory: chronologicalHistory,
    sections: sectionsList,
    spotlights: {
      totalSpotlights: spotlights.length,
      totalViews: totalAllSpotlightViews,
      averageViews: Math.round(totalAllSpotlightViews / Math.max(1, spotlights.length)),
      items: enrichedSpotlights,
    },
    topArticles,
  };
}
